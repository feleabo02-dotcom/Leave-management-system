<x-layouts.erp :title="'Asset Maintenance'">
    <div class="flex items-center justify-between mb-6">
        <div>
            <h1 class="text-2xl font-bold text-gray-900">Asset Maintenance</h1>
            <p class="text-sm text-gray-500 mt-0.5">Track maintenance activities for all assets.</p>
        </div>
        <div class="flex gap-2">
            <button onclick="document.getElementById('addMaintenanceModal').classList.remove('hidden')" class="px-4 py-2 bg-indigo-600 text-white rounded-lg text-sm font-medium hover:bg-indigo-700 transition shadow-sm flex items-center gap-2">
                <i class="ph ph-plus"></i> Schedule Maintenance
            </button>
        </div>
    </div>
            ...
            <table class="w-full text-left border-collapse">
                <thead>
                    <tr class="bg-gray-50 text-[10px] uppercase tracking-wider text-gray-500 font-bold">
                        ...
                        <th class="px-5 py-4 border-b border-gray-100">Completed Date</th>
                        <th class="px-5 py-4 border-b border-gray-100">Cost</th>
                ...
                <tbody class="divide-y divide-gray-50">
                    @forelse($maintenances as $maint)
                        <tr class="hover:bg-gray-50 transition">
                            <td class="px-5 py-4 text-sm font-bold text-gray-900">{{ $maint->asset->name ?? '—' }}</td>
                            <td class="px-5 py-4 text-sm text-gray-600">{{ $maint->title }}</td>
                            <td class="px-5 py-4 text-sm capitalize text-gray-600">{{ $maint->type }}</td>
                            <td class="px-5 py-4">
                                <span class="px-2 py-0.5 text-[10px] font-bold rounded-full uppercase
                                    @if($maint->status === 'completed') bg-green-100 text-green-700
                                    @elseif($maint->status === 'in_progress') bg-orange-100 text-orange-700
                            ...
                            <td class="px-5 py-4 text-right">
                                @if($maint->status !== 'completed')
                                    <form action="{{ route('assets.maintenance.complete', $maint) }}" method="POST" class="inline">
                                        @csrf @method('PATCH')
                                        <button type="submit" class="text-gray-400 hover:text-green-600 transition p-1.5" title="Complete"><i class="ph ph-check-circle text-lg"></i></button>
                                    </form>
                                @endif
                            </td>
                        </tr>
                    @empty
                        <tr>
                            <td colspan="9" class="px-5 py-12 text-center text-gray-400 text-sm">No maintenance records found.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
    ...
    <div id="addMaintenanceModal" class="hidden fixed inset-0 z-50 overflow-y-auto">
        <div class="flex items-end justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
            <div class="fixed inset-0 bg-gray-500 bg-opacity-75 transition-opacity" onclick="document.getElementById('addMaintenanceModal').classList.add('hidden')"></div>
            <span class="hidden sm:inline-block sm:align-middle sm:h-screen" aria-hidden="true">&#8203;</span>
            <div class="inline-block align-bottom bg-white rounded-lg text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg sm:w-full">
                <form action="{{ route('assets.maintenance.store') }}" method="POST">
                    @csrf
                    <div class="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                        <h3 class="text-lg font-bold text-gray-900 mb-4">Schedule Maintenance</h3>
                        <div class="grid grid-cols-2 gap-4">
                            <div class="col-span-2">
                                <label class="block text-xs font-bold text-gray-500 uppercase mb-1">Asset</label>
                                <select name="asset_id" required class="w-full px-3 py-2 border border-gray-200 rounded-lg text-sm">
                                    @foreach($assets as $a)
                    ...
                    <div class="bg-gray-50 px-4 py-3 sm:px-6 sm:flex sm:flex-row-reverse gap-2">
                        ...
                        <button type="button" onclick="document.getElementById('addMaintenanceModal').classList.add('hidden')" class="mt-3 w-full inline-flex justify-center rounded-lg border border-gray-300 shadow-sm px-4 py-2 bg-white text-base font-medium text-gray-700 hover:bg-gray-50 sm:mt-0 sm:w-auto sm:text-sm">Cancel</button>
                    </div>
                </form>
            </div>
        </div>
