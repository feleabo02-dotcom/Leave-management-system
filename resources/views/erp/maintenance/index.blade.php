<x-layouts.erp :title="'Maintenance'">
    <div class="flex items-center justify-between mb-6">
        <div>
            <h1 class="text-2xl font-bold text-gray-900">Maintenance</h1>
            <p class="text-sm text-gray-500 mt-0.5">Manage equipment and maintenance requests.</p>
        </div>
        <div class="flex gap-2">
            <button onclick="document.getElementById('addEquipmentModal').classList.remove('hidden')" class="px-4 py-2 bg-indigo-600 text-white rounded-lg text-sm font-medium hover:bg-indigo-700 transition shadow-sm flex items-center gap-2">
    ...
    <div class="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
        ...
        <div class="bg-white p-4 rounded-xl border border-gray-200 shadow-sm">
            ...
            <p class="text-2xl font-bold text-red-600">{{ \App\Models\MaintenanceRequest::whereIn('status', ['pending', 'in_progress'])->count() }}</p>
        </div>
    </div>
    ...
    <div class="bg-white border border-gray-200 rounded-xl shadow-sm overflow-hidden mb-6">
        <div class="px-5 py-4 border-b border-gray-100 bg-gray-50">
            <h3 class="font-semibold text-gray-800 flex items-center gap-2">
                <i class="ph ph-toolbox text-indigo-500"></i> Equipment
            </h3>
        </div>
            ...
            <table class="w-full text-left border-collapse">
                <thead>
                    <tr class="bg-gray-50 text-[10px] uppercase tracking-wider text-gray-500 font-bold">
                        ...
                        <th class="px-5 py-4 border-b border-gray-100">Location</th>
                ...
                <tbody class="divide-y divide-gray-50">
                    @forelse($equipment as $item)
                        <tr class="hover:bg-gray-50 transition">
                            ...
                            <td class="px-5 py-4 text-sm text-gray-600">{{ $item->category->name }}</td>
                            <td class="px-5 py-4 text-sm text-gray-600">{{ $item->code }}</td>
                            <td class="px-5 py-4">
                                <span class="px-2 py-0.5 text-[10px] font-bold rounded-full uppercase
                                    @if($item->status === 'operating') bg-green-100 text-green-700
                                    @elseif($item->status === 'under_maintenance') bg-orange-100 text-orange-700
                                    @else bg-red-100 text-red-700
                                    @endif">
                                    {{ str_replace('_', ' ', $item->status) }}
                                </span>
                            </td>
                            <td class="px-5 py-4 text-sm text-gray-600">{{ $item->location ?? '—' }}</td>
                            <td class="px-5 py-4 text-right">
                                <a href="{{ route('maintenance.show', $item) }}" class="text-gray-400 hover:text-indigo-600 transition p-1.5 inline-block"><i class="ph ph-eye text-lg"></i></a>
                            </td>
                        </tr>
                    @empty
                        <tr>
                            <td colspan="6" class="px-5 py-12 text-center text-gray-400 text-sm">No equipment registered yet.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
    ...
    <div class="bg-white border border-gray-200 rounded-xl shadow-sm overflow-hidden">
        <div class="px-5 py-4 border-b border-gray-100 bg-gray-50 flex items-center justify-between">
            <h3 class="font-semibold text-gray-800 flex items-center gap-2">
                <i class="ph ph-clipboard-text text-indigo-500"></i> Maintenance Requests
            </h3>
            <button onclick="document.getElementById('addRequestModal').classList.remove('hidden')" class="text-xs font-medium text-indigo-600 hover:underline">+ New Request</button>
        </div>
        <div class="overflow-x-auto">
            <table class="w-full text-left border-collapse">
                <thead>
                    <tr class="bg-gray-50 text-[10px] uppercase tracking-wider text-gray-500 font-bold">
                        ...
                        <th class="px-5 py-3 border-b border-gray-100">Equipment</th>
                        <th class="px-5 py-3 border-b border-gray-100">Priority</th>
                        <th class="px-5 py-3 border-b border-gray-100">Stage</th>
                ...
                <tbody class="divide-y divide-gray-50">
                    @forelse($requests as $req)
                        <tr class="hover:bg-gray-50 transition">
                            ...
                            <td class="px-5 py-3 text-sm text-gray-600">{{ $req->equipment->name }}</td>
                            <td class="px-5 py-3">
                                <span class="px-2 py-0.5 text-[10px] font-bold rounded-full uppercase
                    ...
                    @empty
                        <tr>
                            <td colspan="6" class="px-5 py-6 text-center text-sm text-gray-400">No maintenance requests.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
    ...
    <div id="addEquipmentModal" class="hidden fixed inset-0 z-50 overflow-y-auto">
        <div class="flex items-end justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
            <div class="fixed inset-0 bg-gray-500 bg-opacity-75 transition-opacity" onclick="document.getElementById('addEquipmentModal').classList.add('hidden')"></div>
            <span class="hidden sm:inline-block sm:align-middle sm:h-screen" aria-hidden="true">&#8203;</span>
            <div class="inline-block align-bottom bg-white rounded-lg text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg sm:w-full">
                <form action="{{ route('maintenance.equipment.store') }}" method="POST">
                    @csrf
                    <div class="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                        <h3 class="text-lg font-bold text-gray-900 mb-4">Add Equipment</h3>
                        <div class="grid grid-cols-2 gap-4">
                            ...
                            <div>
                                ...
                                <select name="location" class="w-full px-3 py-2 border border-gray-200 rounded-lg text-sm">
                            ...
                            <div>
                                ...
                                <input type="date" name="purchase_date" class="w-full px-3 py-2 border border-gray-200 rounded-lg text-sm">
                            </div>
                            <div>
                                <label class="block text-xs font-bold text-gray-500 uppercase mb-1">Purchase Cost</label>
                                <input type="number" step="0.01" name="purchase_cost" class="w-full px-3 py-2 border border-gray-200 rounded-lg text-sm">
                            </div>
                    ...
                    <div class="bg-gray-50 px-4 py-3 sm:px-6 sm:flex sm:flex-row-reverse gap-2">
                        ...
                        <button type="button" onclick="document.getElementById('addEquipmentModal').classList.add('hidden')" class="mt-3 w-full inline-flex justify-center rounded-lg border border-gray-300 shadow-sm px-4 py-2 bg-white text-base font-medium text-gray-700 hover:bg-gray-50 sm:mt-0 sm:w-auto sm:text-sm">Cancel</button>
                    </div>
                </form>
            </div>
        </div>
