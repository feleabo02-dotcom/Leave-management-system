<x-layouts.erp :title="'Maintenance Requests'">
    <div class="flex items-center justify-between mb-6">
        <div>
            <h1 class="text-2xl font-bold text-gray-900">Maintenance Requests</h1>
            <p class="text-sm text-gray-500 mt-0.5">All equipment maintenance requests.</p>
        </div>
        <div class="flex gap-2">
            <button onclick="document.getElementById('addRequestModal').classList.remove('hidden')" class="px-4 py-2 bg-indigo-600 text-white rounded-lg text-sm font-medium hover:bg-indigo-700 transition shadow-sm flex items-center gap-2">
            ...
            <table class="w-full text-left border-collapse">
                <thead>
                    <tr class="bg-gray-50 text-[10px] uppercase tracking-wider text-gray-500 font-bold">
                        ...
                        <th class="px-5 py-4 border-b border-gray-100">Equipment</th>
                        ...
                        <th class="px-5 py-4 border-b border-gray-100">Scheduled</th>
                ...
                <tbody class="divide-y divide-gray-50">
                    @forelse($requests as $req)
                        <tr class="hover:bg-gray-50 transition">
                            <td class="px-5 py-4 text-sm font-bold text-gray-900">{{ $req->name }}</td>
                            <td class="px-5 py-4 text-sm text-gray-600">{{ $req->equipment->name }}</td>
                            <td class="px-5 py-4">
                            ...
                            <td class="px-5 py-4 text-sm capitalize text-gray-600">{{ $req->category ?? '—' }}</td>
                            <td class="px-5 py-4">
                                <span class="px-2 py-0.5 text-[10px] font-bold rounded-full uppercase
                            ...
                            <td class="px-5 py-4 text-sm text-gray-600">{{ $req->scheduled_date?->format('M d, Y') ?? '—' }}</td>
                            <td class="px-5 py-4 text-right">
                                <div class="flex justify-end gap-1">
                                    @if($req->status === 'pending')
                                        <form action="{{ route('maintenance.requests.status', $req) }}" method="POST" class="inline">
                                            @csrf @method('PATCH')
                                            <input type="hidden" name="status" value="in_progress">
                                            <button type="submit" class="text-gray-400 hover:text-orange-600 transition p-1.5" title="Start"><i class="ph ph-play text-lg"></i></button>
                                        </form>
                                    @endif
                    ...
                    @empty
                        <tr>
                            <td colspan="7" class="px-5 py-12 text-center text-gray-400 text-sm">No maintenance requests found.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
