<x-layouts.erp :title="'New Purchase Order'">
    <div class="flex items-center gap-4 mb-6">
        ...
        <div>
            ...
            <p class="text-sm text-gray-500 mt-0.5">Fill in the details to generate a new PO.</p>
        </div>
    </div>
    ...
    <form action="{{ route('procurement.store') }}" method="POST">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div class="lg:col-span-3">
                <div class="bg-white border border-gray-200 rounded-xl p-6 shadow-sm">
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                        ...
                        <div>
                            <label class="block text-xs font-bold text-gray-500 uppercase mb-1">Deliver To (Warehouse)</label>
                                    ...
                                    <td class="px-5 py-4">
                                        <select name="items[0][product_id]" required class="w-full border-none bg-transparent text-sm focus:ring-0">
                                            ...
                                            @foreach($products as $prod)
                                                <option value="{{ $prod->id }}" data-price="{{ $prod->cost }}">{{ $prod->name }}</option>
                                            @endforeach
                                        </select>
                                    </td>
                                    <td class="px-5 py-4">
            ...
            <div class="lg:col-span-3 flex justify-end gap-3">
                <a href="{{ route('procurement.index') }}" class="px-6 py-2 border border-gray-200 text-gray-700 rounded-lg text-sm font-medium hover:bg-gray-50 transition">Cancel</a>
                <button type="submit" class="px-6 py-2 bg-indigo-600 text-white rounded-lg text-sm font-bold hover:bg-indigo-700 transition shadow-lg shadow-indigo-200">Create Purchase Order</button>
            </div>
        </div>
    </form>
